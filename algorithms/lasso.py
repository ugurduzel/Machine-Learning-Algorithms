...
###
#	Multivariate Lasso (Least Absolute Shrinkage and Selection Operator) Regression
#	Basically linear model trained 
#	with least squares loss function and L1 regularization
###
class Lasso:
	...
	def gradientDescent(self):
	    ...
	    for iter in range(self.max_epochs):
	        ...
	        self.weights = self.weights * (1 - self.lr*self.alpha/(2*self.m)) -  ((self.lr/self.m) * self.X.T.dot((hypothesis - self.Y))).reshape(1,self.X.shape[1])
...
if __name__ == "__main__":
	model = Lasso(alpha = 0.5, learning_rate = 0.0001, max_epochs = 300, verbose = True)
	m = 10000
	X = []
	Y = []
	for i in range(0,m):
	...
	model2 = sklearn.linear_model.Lasso(alpha = 0.5, fit_intercept=False)
	model2.fit(X,Y.T)
